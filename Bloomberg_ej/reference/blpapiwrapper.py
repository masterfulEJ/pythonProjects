...
class BLP():
    ...
    def bdp(self, strSecurity='US900123AL40 Govt', strData='PX_LAST', strOverrideField='',
            strOverrideValue=''):
        ...
        try:
            output = blpapi.event.MessageIterator(event).__next__().getElement(
                SECURITY_DATA).getValueAsElement(0).getElement(FIELD_DATA).getElementAsString(strData)
            if output == '#N/A':
    ...
    def bdh(self, strSecurity='SPX Index', strData='PX_LAST', startdate=datetime.date(2014, 1, 1),
        ...
        fieldDataArray = blpapi.event.MessageIterator(
            event).__next__().getElement(SECURITY_DATA).getElement(FIELD_DATA)
        fieldDataList = [fieldDataArray.getValueAsElement(
            i) for i in range(0, fieldDataArray.numValues())]
    ...
    def bdhOHLC(self, strSecurity='SPX Index', startdate=datetime.date(2014, 1, 1),
                enddate=datetime.date(2014, 1, 9), periodicity='DAILY'):
        return(self.bdh(strSecurity, ['PX_OPEN', 'PX_HIGH', 'PX_LOW', 'PX_LAST'],
...
class BLPTS():
    ...
    def fillRequest(self, securities, fields, **kwargs):
        ...
        if 'startDate' in kwargs:
            self.request = self.refDataSvc.createRequest(
    ...
    def get(self, newSecurities=[], newFields=[], **kwargs):
        ...
        while True:
            ...
            if event.eventType() in [blpapi.event.Event.RESPONSE, blpapi.event.Event.PARTIAL_RESPONSE]:
                responseSize = blpapi.event.MessageIterator(
                    event).__next__().getElement(SECURITY_DATA).numValues()

                for i in range(0, responseSize):

                    if 'startDate' in self.kwargs:
                        # HistoricalDataRequest
                        output = blpapi.event.MessageIterator(
                            event).__next__().getElement(SECURITY_DATA)
                        security = output.getElement(
                            SECURITY).getValueAsString()
                        fieldDataArray = output.getElement(FIELD_DATA)
                        fieldDataList = [fieldDataArray.getValueAsElement(
                            i) for i in range(0, fieldDataArray.numValues())]
                        dates = map(lambda x: x.getElement(
                            DATE).getValueAsString(), fieldDataList)
                        outDF = pandas.DataFrame(
                            index=dates, columns=self.fields)
                        ...
                        for field in self.fields:
                            ...
                            for row in fieldDataList:
                                if row.hasElement(field):
                                    data.append(row.getElement(
                                        field).getValueAsFloat())
                                else:
                                    data.append(pandas.np.nan)

                            outDF[field] = data
                            # update one security one field
                            self.updateObservers(
                                security=security, field=field, data=outDF)

                        # update one security all fields
                        self.updateObservers(
                            security=security, field='ALL', data=outDF)

                    else:
                        # ReferenceDataRequest
                        output = blpapi.event.MessageIterator(event).__next__(
                        ).getElement(SECURITY_DATA).getValueAsElement(i)
                        ...
                        for j in range(0, n_elmts):
                            ...
                            # update one security one field
                            self.updateObservers(
                                security=security, field=field, data=outData)
                            self.output.loc[security, field] = outData

                        if n_elmts > 0:
                            self.updateObservers(security=security, field='ALL', data=self.output.loc[
                                                 security])  # update one security all fields
                        else:
...
class BLPStream(threading.Thread):
    ...
    def __init__(self, strSecurityList=['ESM5 Index', 'VGM5 Index'], strDataList=['BID', 'ASK'], floatInterval=0, intCorrIDList=[0, 1]):
        ...
        for (security, intCorrID) in zip(self.strSecurityList, self.intCorrIDList):
            self.subscriptionList.add(
                security, self.strDataList, "interval=" + str(floatInterval), blpapi.CorrelationId(intCorrID))

        self.output = pandas.DataFrame(
            index=self.strSecurityList, columns=self.strDataList)
    ...
    def handleDataEvent(self, event):
        ...
        if output.hasElement(EVENT_TIME):
            self.lastUpdateTimeBlmbrg = output.getElement(
                EVENT_TIME).toString()
        ...
        for field in self.strDataList:
            if output.hasElement(field):
                ...
                try:
                    ...
                    data = pandas.np.nan
                    # ,output.getElement(field).getValueAsString() # this can still error if field is there but is empty
                    print('error: ', security, field)
        ...
        # It can happen that you get an event without the data behind the
        # event!
        self.updateObservers(time=self.lastUpdateTime, security=security,
                             field='ALL', corrID=corrID, data=0, bbgTime=self.lastUpdateTimeBlmbrg)
        # if not isParsed:
        #     print output.toString()
...
def simpleHistoryRequest(securities=[], fields=[], startDate=datetime.datetime(2015, 1, 1), endDate=datetime.datetime(2016, 1, 1), periodicity='DAILY'):
    ...
    '''
    blpts = BLPTS(securities, fields, startDate=startDate,
                  endDate=endDate, periodicity=periodicity)
...
class ObserverStreamExample(Observer):
    def update(self, *args, **kwargs):
        output = kwargs['time'].strftime("%Y-%m-%d %H:%M:%S") + ' received ' + kwargs[
            'security'] + ' ' + kwargs['field'] + '=' + str(kwargs['data'])
        output = output + '. CorrID ' + \
            str(kwargs['corrID']) + ' bbgTime ' + kwargs['bbgTime']
        print(output)


def streamPatternExample():
    stream = BLPStream('ESM5 Index', ['BID', 'ASK'], 0, 1)
    # stream=BLPStream('XS1151974877 CORP',['BID','ASK'],0,1) #Note that for a
    # bond only BID gets updated even if ASK moves.
...
def BLPTSExample():
    result = BLPTS(['XS0316524130 Corp', 'US900123CG37 Corp'], [
                   'PX_BID', 'INT_ACC', 'DAYS_TO_NEXT_COUPON'])
    result.get()
    print(result.output)
    result.closeSession()
