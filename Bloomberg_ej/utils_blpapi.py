...
@author: ryan + (modified by) Eunjae Jang
"""
import blpapi
import pandas as pd
import numpy as np
...
class BLP:
    ...
    def bdh(self, securities, fields, startDate, endDate, dictOverrideField={}, **kwargs):
        ...
        response = self.sendRequest(
            'HistoricalData', securities, fields, dictOverrideField, defaults)
        ...
        for msg in response:
            ...
            fieldDataList = [fieldData.getValueAsElement(
                i) for i in range(fieldData.numValues())]

            df = DataFrame()

            for fld in fieldDataList:
                for v in [fld.getElement(i) for i in range(fld.numElements()) if fld.getElement(i).name() != 'date']:
                    df.ix[fld.getElementAsDatetime(
                        'date'), str(v.name())] = v.getValue()
    ...
    def bdp(self, securities, fields, dictOverrideField={}, **kwargs):
        ...
        """
        response = self.sendRequest(
            'ReferenceData', securities, fields, dictOverrideField, kwargs)
        ...
        for msg in response:
            ...
            securityDataList = [securityData.getValueAsElement(
                i) for i in range(securityData.numValues())]

            for sec in securityDataList:
                fieldData = sec.getElement('fieldData')
                fieldDataList = [fieldData.getElement(
                    i) for i in range(fieldData.numElements())]

                for fld in fieldDataList:
                    data.ix[sec.getElementAsString('security'), str(
                        fld.name())] = fld.getValue()
    ...
    def bds(self, securities, fields, dictOverrideField={}, **kwargs):
        ...
        if type(securities) == str:
            ...
            data = pd.concat(data, keys=keys, axis=0, names=[
                             'Security', data[0].index.name])
        ...
        while True:
            ...
            for msg in event:
                if msg.hasElement('responseError'):
                    raise RequestError(msg.getElement(
                ...
                if msg.hasElement('securityData'):
                    if msg.getElement('securityData').hasElement('fieldExceptions') and (msg.getElement('securityData').getElement('fieldExceptions').numValues() > 0):
                        raise RequestError(msg.getElement('securityData').getElement(
                            'fieldExceptions'), 'Field Error')
...
def main():
    ...
    try:
        ...
        print(blp.bdh('BMO CN Equity',
                      'PX_LAST', '20141231', '20150131'))

        # Requesting multiple fields returns a DataFrame with multiple columns.
        # Dates may also be passed as a datetime.
        print(blp.bdh('BNS CN Equity', [
              'PX_LAST', 'PX_VOLUME'], datetime(2014, 12, 31), datetime(2015, 1, 31)))

        # Requesting multiple securities returns a DataFrame with a MultiIndex.
        print(blp.bdh(['CM CN Equity', 'NA CN Equity'], [
              'PX_LAST', 'PX_VOLUME'], '20141231', '20150131'))

        # You may force any DataFrame to include a MultiIndex by passing the
        # arguments as lists.
        print(blp.bdh(['NA CN Equity'], [
              'PX_LAST'], '20141231', '20150131'))

        # Keyword arguments are added to the request, allowing you to perform
        # advanced queries.
        print(blp.bdh('TD CN Equity', 'PCT_CHG_INSIDER_HOLDINGS',
                      '20141231', '20150131', periodicitySelection='WEEKLY'))
        ...
        with BLP() as blp:
            ...
            # = bdp() Examples =
            # =============================
            # Requesting a single security/field will return the single value,
            # not a DataFrame.
            print(blp.bdp('BBD/B CN Equity', 'GICS_SECTOR'))

            # Requesting multiple securities or fields will return a DataFrame.
            print(blp.bdp(['CNR CN Equity', 'CP CN Equity'], [
                  'SECURITY_NAME_REALTIME', 'LAST_PRICE']))

            # You may force any request to return a DataFrame by passing the
            # arguments as lists.
